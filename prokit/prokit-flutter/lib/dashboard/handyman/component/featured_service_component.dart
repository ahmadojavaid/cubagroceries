...
class FeaturedService extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        titleRowWidget(handymanAppString.featuredService, handymanAppString.viewAll),
        SizedBox(height: 8),
        SizedBox(
          height: 280,
          child: ListView.builder(
            scrollDirection: Axis.horizontal,
            itemCount: featuredServiceCards.length,
            padding: const EdgeInsets.only(left: 20),
            itemBuilder: (context, index) {
              final card = featuredServiceCards[index];
              return Container(
                width: 220,
                margin: EdgeInsets.only(
                  right: index == (featuredServiceCards.length - 1) ? 20 : 16,
                ),
